...
function agricole_mytheme_comment($comment, $args, $depth) {
    ?>
			...
			<div class="comment-content cmsmasters_comment_item_cont">
				...
				<div class="cmsmasters_comment_item_content">
					...
					if ($comment->comment_approved == '0') {
						echo '<p>' . 
							'<em>' . esc_html__('Your comment is awaiting moderation.', 'agricole') . '</em>' . 
						'</p>';
					}
					?>
				</div>
				<?php
					comment_reply_link(array_merge($args, array( 
						...
						'reply_text' => esc_attr__('Reply', 'agricole') 
					)));
					
					edit_comment_link(esc_attr__('Edit', 'agricole'), '', '');
				?>
			</div>
        </div>
    <?php 
