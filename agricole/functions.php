...
if (!function_exists('agricole_system_fonts_list')) {
	function agricole_system_fonts_list() {
		$fonts = array( 
...
if (!function_exists('agricole_text_decoration_list')) {
	function agricole_text_decoration_list() {
		$list = array( 
			'none' => esc_html__('none', 'agricole'), 
			'underline' => esc_html__('underline', 'agricole'), 
			'overline' => esc_html__('overline', 'agricole'), 
			'line-through' => esc_html__('line-through', 'agricole'), 
		);
...
if (!function_exists('agricole_custom_color_schemes_list')) {
	function agricole_custom_color_schemes_list() {
		$list = array( 
			'first' => esc_html__('Custom 1', 'agricole'), 
			'second' => esc_html__('Custom 2', 'agricole'), 
			'third' => esc_html__('Custom 3', 'agricole') 
		);
		
		
		return apply_filters('agricole_custom_color_schemes_list_filter', $list);
	}
}
...
if (!function_exists('agricole_locate_template')) {
	function agricole_locate_template($template_names, $require_once = true, $load = true) {
		$located = '';
...
// Theme Style
define('CMSMASTERS_THEME_STYLE', (CMSMASTERS_THEME_STYLE_COMPATIBILITY && get_option('cmsmasters_agricole_theme_style') ? get_option('cmsmasters_agricole_theme_style') : ''));
...
if (!function_exists('agricole_all_theme_styles')) {
	function agricole_all_theme_styles() {
		$out = array( 
			'Default|', 
			'Theme Style 1|1', 
			'Theme Style 2|2' 
...
if (!function_exists('agricole_all_color_schemes_list')) {
	function agricole_all_color_schemes_list() {
		$list = array( 
			'default' => 		esc_html__('Default', 'agricole'), 
			'header' => 		esc_html__('Header', 'agricole'), 
			'navigation' => 	esc_html__('Navigation', 'agricole'), 
			'header_top' => 	esc_html__('Header Top', 'agricole'), 
			'footer' => 		esc_html__('Footer', 'agricole') 
		);
...
if (!function_exists('agricole_load_theme_textdomain')) {
	function agricole_load_theme_textdomain() {
		load_theme_textdomain('agricole', CMSMASTERS_VARS . '/languages');
	}
}

// Load Theme Local File Action
if (!has_action('after_setup_theme', 'agricole_load_theme_textdomain')) {
	add_action('after_setup_theme', 'agricole_load_theme_textdomain');
}



// Framework Activation & Data Import
if (!function_exists('agricole_theme_activation')) {
	function agricole_theme_activation() {
		if (get_option('cmsmasters_active_theme') != 'agricole') {
			add_option('cmsmasters_active_theme', 'agricole', '', 'yes');
...
add_action('after_switch_theme', 'agricole_theme_activation');



// Framework Deactivation
if (!function_exists('agricole_theme_deactivation')) {
	function agricole_theme_deactivation() {
		delete_option('cmsmasters_active_theme');
	}
}

add_action('switch_theme', 'agricole_theme_deactivation');



// Plugin Activation Regenerate Styles
if (!function_exists('agricole_plugin_activation')) {
	function agricole_plugin_activation($plugin, $network_activation) {
...
add_action('activated_plugin', 'agricole_plugin_activation', 10, 2);


if (!function_exists('agricole_plugin_activation_regenerate')) {
	function agricole_plugin_activation_regenerate() {
...
add_action('init', 'agricole_plugin_activation_regenerate');


function agricole_run_reinit_import_options($post_id, $key, $value) {
	if (!get_post_meta($post_id, 'cmsmasters_heading', true)) {
		$custom_post_meta_fields = agricole_get_custom_all_meta_fields();
...
add_action('import_post_meta', 'agricole_run_reinit_import_options', 10, 3);
